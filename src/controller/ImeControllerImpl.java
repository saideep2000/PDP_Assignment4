...
public class ImeControllerImpl implements ImeController {
  ...
  /**
   * This method will send the path to model for loading and put it in hashmap by the name given.
   */
  private void load() {
  ...
  private void brighten() {
    if (commandsList.length != 4) {
      ...
      if (checkIfFileExists(commandsList[2])) {
        ListOfImages.put(commandsList[3], ListOfImages.get(commandsList[2])
                .brighten(Integer.valueOf(commandsList[1])));
  ...
  /**
   * This method will send the strings to the view to be displayed.
   *
